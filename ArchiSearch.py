...
class AFSArchitectureSearch:
    ...
    def get_architecture_configs(self):
        ...
        return {
            ...
            'e_node': [32, 64, 128, 256, 512],  # Extraction network size
            'a_node': [2, 4, 8, 16, 32],  # Attention network output size
            
            # Non-linearity configurations
            'extract_activation': ['tanh'],  # Extraction network activation
            'hidden_activation': ['relu'],  # Learning module activation
            ...
            # Training Params
            'batch_size': [32, self.train_size[0]],
    ...
    def train_model(self, model, train_loader, val_loader, config):
        ...
        for epoch in range(config['num_epochs']):
            ...
            print(f"===================Epoch {epoch_count} | Loss: {val_loss:.6f} | Accuracy: {val_accuracy:.3f}===================")

            # Learning rate scheduling
            scheduler.step(val_loss)

            # Get feature weights
            A = model.get_feature_weights(train_loader, self.device)

            # Print Attention mean and std (only values)
            print(f"Attention mean: {A.mean().item():.4f} | std: {A.std().item():.4f}")

            # Get top 10 feature weights
            top_values, top_indices = torch.topk(A, 5)

            # Print only values without the extra 'torch.return_types.topk(...)'
            print("Top 5 feature weights:", top_values.cpu().numpy())
    ...
    def evaluate_model(self, model, data_loader):
        ...
        print("model attention", model.get_feature_weights(data_loader, self.device))
        print("Computed attention", attention_tensor)
        print("Sorted attention", sorted_attention)
